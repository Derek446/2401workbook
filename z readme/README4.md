...
{% for item in items %}
   ...
   {% else %}
       {{forloop.counter}}. {{ item.title }} - {{ item.rating }} - Reviews were not bad
   {% endif %}
{% endfor %}
""")
```
...
```
Movies ratings from 0 to 10
    1. The Matrix - 9 - Reviews were not bad
    2. Inception - 8 - Reviews were not bad
    3. The Godfather - 10 - Well Reviewed
    4. Twilight - 1 - Reviews were not bad
```
